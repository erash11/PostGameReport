...
# BU Logo
message("Loading Baylor logo...")
bu_logo_url <- team_info$logo[team_info$school == "Baylor"]
if (length(bu_logo_url) == 0 || is.na(bu_logo_url) || bu_logo_url == "") {
  warning("❌ Baylor logo URL not found in team_info - using green placeholder")
  bu_logo <- image_blank(150, 150, color = "#003015")
} else {
  message("✓ Baylor logo URL: ", bu_logo_url)
  tryCatch({
    bu_logo <- image_read(bu_logo_url)
    message("✓ Baylor logo loaded successfully")
  }, error = function(e) {
    warning("❌ Failed to load Baylor logo from URL: ", e$message, " - using placeholder")
    bu_logo <<- image_blank(150, 150, color = "#003015")
  })
}

# Opponent Logo
message("Loading ", opp_name, " logo...")
opp_logo_url <- team_info$logo[team_info$school == opp_name]
if (length(opp_logo_url) == 0 || is.na(opp_logo_url) || opp_logo_url == "") {
  warning("❌ Opponent logo URL not found for: ", opp_name, " - using colored placeholder")
  opp_logo <- image_blank(150, 150, color = opp_color)
} else {
  message("✓ Opponent logo URL: ", opp_logo_url)
  tryCatch({
    opp_logo <- image_read(opp_logo_url)
    message("✓ Opponent logo loaded successfully")
  }, error = function(e) {
    warning("❌ Failed to load opponent logo from URL: ", e$message, " - using placeholder")
    opp_logo <<- image_blank(150, 150, color = opp_color)
  })
}
...
win_plot_data %>%
  ...
  # Position logos: Baylor always on TOP (near 100%), opponent on BOTTOM (near 0%)
  # Note: bu_logo and opp_logo were loaded earlier in the document
...
`r generate_wp_analysis(win_plot_data, opp_name, wp_column, current_game_data)`
...
`r generate_offense_analysis(current_game_off_agg, bu_season_off_agg, power_four_off_agg, current_game_off_data)`
...
`r generate_defense_analysis(current_game_def_agg, bu_season_def_agg, power_four_def_agg, current_game_def_data)`
