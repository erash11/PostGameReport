...
if (length(bu_logo_wp) == 0 || is.na(bu_logo_wp) || bu_logo_wp == "") {
  ...
  bu_logo <- image_blank(150, 150, color = "#003015")
} else {
  message("✓ Baylor logo URL: ", bu_logo_wp)
  bu_logo <- image_read(bu_logo_wp)
  message("✓ Baylor logo loaded successfully")
}
...
if (length(opp_logo_wp) == 0 || is.na(opp_logo_wp) || opp_logo_wp == "") {
  ...
  opp_logo <- image_read(opp_logo_wp)
  message("✓ Opponent logo loaded successfully")
}
...
win_plot_data %>%
  ...
  # Position logos: Baylor always on top (above 100%), opponent on bottom (below 0%)
  # Using coordinates relative to win probability scale (0 to 1)
  draw_image(bu_logo, x = -61, y = 1.05, width = 6.67, height = 0.15) +
  draw_image(opp_logo, x = -61, y = -0.20, width = 6.67, height = 0.15) +
