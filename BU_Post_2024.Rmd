...
# Automatically detect winner and show their win probability
is_baylor_home <- current_game_data$home_team[1] == "Baylor"
final_home_score_wp <- max(win_plot_data$home_score, na.rm = TRUE)
final_away_score_wp <- max(win_plot_data$away_score, na.rm = TRUE)
home_team_won <- final_home_score_wp > final_away_score_wp

message("Home team: ", current_game_data$home_team[1], " (", final_home_score_wp, ")")
message("Away team: ", current_game_data$away_team[1], " (", final_away_score_wp, ")")
message("Winner: ", if(home_team_won) "HOME" else "AWAY")

# Select the winning team's win probability column
if (home_team_won) {
  wp_column <- "home_wp_before"
  winner_name <- current_game_data$home_team[1]
} else {
  wp_column <- "away_wp_before"
  winner_name <- current_game_data$away_team[1]
}
...
win_plot_data %>%
  ...
  # Position logos appropriately (winner on top since chart shows winner's WP)
  draw_image(bu_logo, x = -61, y = if(winner_name == "Baylor") 0.88 else -0.12, width = 6.67, height = 2) +
  draw_image(opp_logo, x = -61, y = if(winner_name == "Baylor") -0.12 else 0.88, width = 6.67, height = 2) +
  ...
  labs(x = "Time Remaining (minutes)",                                                # Add labels
    y = "Win Probability",
    title = paste0(winner_name, "'s Win Probability - Baylor vs ", opp_name),
    subtitle = paste0('Showing ', winner_name, "'s chances of winning throughout the game")) + 
