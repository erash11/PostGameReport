...
if (length(opp_logo_url) == 0 || is.na(opp_logo_url) || opp_logo_url == "") {
  warning("Opponent logo not found for: ", opp_name, " - using placeholder")
  # Create a visible placeholder logo with opponent color
  opp_logo <- image_blank(150, 150, color = opp_color)
} else {
  message("✓ Opponent logo found: ", opp_logo_url)
...
# Extract game information for title
final_home_score <- max(current_game_data$home_score, na.rm = TRUE)
final_away_score <- max(current_game_data$away_score, na.rm = TRUE)
is_baylor_home_game <- current_game_data$home_team[1] == "Baylor"
baylor_score <- if(is_baylor_home_game) final_home_score else final_away_score
opponent_score <- if(is_baylor_home_game) final_away_score else final_home_score
did_baylor_win <- baylor_score > opponent_score
...
### Create Next Opp Season-Level Aggregates for Analysis

## Next Opp Season OFF aggregate (simple, for analysis functions)
next_opp_off_agg = next_season_off_data %>%
  filter(!is.na(EPA)) %>%
  filter(!is.na(success)) %>%
...
## Next Opp Season DEF aggregate (simple, for analysis functions)
next_opp_def_agg = next_season_def_data %>%
  filter(!is.na(EPA)) %>%
  filter(!is.na(success)) %>%
  filter(r_p == 'Run' | r_p == 'Pass') %>%
  mutate(grouping = paste(next_name, 'Season')) %>%
...
# Baylor vs `r opp_name` Postgame Report

## `r paste0(baylor_score, "-", opponent_score, " ", if(did_baylor_win) "Victory" else "Loss", if(is_baylor_home_game) " (Home)" else " (Away)")`

<br>

#### The following is a football analytics report of the Baylor vs `r opp_name` game.
...
# Reload logos for WPA chart to ensure they display correctly
message("=== Loading logos for WPA chart ===")
message("Looking for Baylor logo...")
bu_logo_wp <- team_info$logo[team_info$school == "Baylor"]
if (length(bu_logo_wp) == 0 || is.na(bu_logo_wp) || bu_logo_wp == "") {
  warning("❌ Baylor logo not found - using GREEN placeholder")
  bu_logo_for_wp <- image_blank(150, 150, color = "#003015")
  message("Created GREEN placeholder for Baylor")
} else {
  message("✓ Baylor logo URL: ", bu_logo_wp)
  bu_logo_for_wp <- image_read(bu_logo_wp)
  message("✓ Baylor logo loaded successfully (type: ", class(bu_logo_for_wp), ")")
}

message("Looking for ", opp_name, " logo...")
opp_logo_wp <- team_info$logo[team_info$school == opp_name]
if (length(opp_logo_wp) == 0 || is.na(opp_logo_wp) || opp_logo_wp == "") {
  warning("❌ Opponent logo not found for: ", opp_name, " - using colored placeholder")
  opp_logo_for_wp <- image_blank(150, 150, color = opp_color)
  message("Created placeholder with color: ", opp_color)
} else {
  message("✓ Opponent logo URL: ", opp_logo_wp)
  opp_logo_for_wp <- image_read(opp_logo_wp)
  message("✓ Opponent logo loaded successfully (type: ", class(opp_logo_for_wp), ")")
}

# Verify both logos are valid before proceeding
message("Verifying logos before drawing...")
message("  Baylor logo valid: ", !is.null(bu_logo_for_wp))
message("  Opponent logo valid: ", !is.null(opp_logo_for_wp))
message("=== Logos loaded ===")
...
# Always show Baylor's win probability (not the winner's)
is_baylor_home <- current_game_data$home_team[1] == "Baylor"

message("Baylor is playing: ", if(is_baylor_home) "HOME" else "AWAY")

# Select Baylor's win probability column based on home/away
if (is_baylor_home) {
...
win_plot_data %>%
  ...
  # Position logos: Baylor always on top, opponent on bottom
  # Using coordinates that were proven to work: y=0.88 for top, y=-0.12 for bottom
  draw_image(bu_logo_for_wp, x = -61, y = 0.88, width = 6.67, height = 2) +
  draw_image(opp_logo_for_wp, x = -61, y = -0.12, width = 6.67, height = 2) +
  ...
  labs(x = "Time Remaining (minutes)",                                                # Add labels
    y = "Win Probability",
    title = paste0("Baylor's Win Probability - Baylor vs ", opp_name),
    subtitle = "Showing Baylor's chances of winning throughout the game") + 
...
### Week `r params$week + 1`: Baylor vs `r next_name`

##### *Baylor will take on `r next_name` next.*
